...
{
    uint8_t res=0;
//  IIC_Init();     //初始化IIC总线     改后应该初始化SPI1
//  MPU9250_Write_Reg(MPU9250_ADDR,MPU_PWR_MGMT1_REG,0X80);//复位MPU9250
	  MPU9250_Write_Reg(MPU_PWR_MGMT1_REG,0X80);
    LL_mDelay(100);  //延时100ms
    MPU9250_Write_Reg(MPU_PWR_MGMT1_REG,0X00);//唤醒MPU9250
    MPU_Set_Gyro_Fsr(3);					        	//陀螺仪传感器,±2000dps
	  MPU_Set_Accel_Fsr(0);					       	 	//加速度传感器,±2g
    MPU_Set_Rate(50);						       	 	//设置采样率50Hz
  	MPU9250_Write_Reg(MPU_USER_CTRL_REG,0X00);  //I2C主模式关闭
	
//	  MPU9250_Write_Reg(MPU_USER_CTRL_REG,0X20);  //I2C主模式开启
...
//    MPU9250_Write_Reg(I2C_SLV0_CTRL,0x81);//Enable reading data from this slave at the sample rate && 1 Byte length
  	...
  	MPU9250_Write_Reg(MPU_FIFO_EN_REG,0X00);	//关闭FIFO
	
	/************  这里有问题 ，不应该是直通模式， 应该是 I2C Master Mode 模式  ****************/
    MPU9250_Write_Reg(MPU_INTBP_CFG_REG,0X82);//INT引脚低电平有效，开启bypass模式，可以直接读取磁力计    
...
//	  MPU9250_Write_Reg(I2C_SLV0_CTRL ,0x81); //enable IIC	and EXT_SENS_DATA==1 Byte
    ...
    res=MPU9250_Read_Reg(MPU_DEVICE_ID_REG);  //读取MPU6500的ID
    if(res==MPU6500_ID1||res==MPU6500_ID2) 				//器件ID正确
    {
			  printff(USART3, "Read MPU6500 ID OK!!!\r\n",strlen("Read MPU6500 ID OK!!!\r\n"));	   
        MPU9250_Write_Reg(MPU_PWR_MGMT1_REG,0X01);  	//设置CLKSEL,PLL X轴为参考
        MPU9250_Write_Reg(MPU_PWR_MGMT2_REG,0X00);  	//加速度与陀螺仪都工作
...
{
	return MPU9250_Write_Reg(MPU_GYRO_CFG_REG,(fsr<<3)|3);//设置陀螺仪满量程范围  
}
...
uint8_t MPU_Get_Magnetometer(short *mx,short *my,short *mz)
{
	...
	MPU9250_Write_Reg(MAG_CNTL1,0X11); //AK8963每次读完以后都需要重新设置为单次测量模式
    return res;;
}
