...
#include "invmpuu.h"
#include "mpl.h"
#include "invensensee.h"
#include "invensenseadv.h"
#include "databuilder.h"
#include "emploutputs.h"
#include "mltypess.h"
#include "mpuu.h"
#include "logg.h"
#include "packett.h"
...
#include "mmpu9250.h"
#include "mp9250.h"
#include "fusion9axis.h"
#include "compassveccal.h"
#include "magdisturb.h"
...
static int set_int_enable(unsigned char enable)
{
    ...
    if (st.chip_cfg.dmp_on) {
        if (enable)
            ...
            tmp = 0x00;
        if (MPU9250_Write_LEN( st.reg->int_enable, 1, &tmp))
            return -1;
...
int mpu_read_reg(unsigned char reg, unsigned char *data)
{
    ...
    if (reg >= st.hw->num_reg)
        return -1;
    return MPU_Read_LEN( reg, 1, data);
}
...
int mpu_init(struct int_param_s *int_param)
{
    ...
    data[0] = BIT_RESET;
//	printff(USART3, "0!!!\r\n",strlen("0!!!\r\n"));	   
    if (MPU9250_Write_Reg( st.reg->pwr_mgmt_1, data[0]))
        return -1;
    LL_mDelay(100);
//printff(USART3, "1!!!\r\n",strlen("1!!!\r\n"));	   
    /* Wake up chip. */
   ...
   st.chip_cfg.accel_half = 0;
    ...
    if (MPU9250_Write_Reg( st.reg->accel_cfg2,  data[0]))
        return -1;
#endif
...
int mpu_lp_accel_mode(unsigned short rate)
{
    ...
    if (!rate) {
        ...
        tmp[0] = 0;
        tmp[1] = BIT_STBY_XYZG;
        if (MPU9250_Write_LEN(st.reg->pwr_mgmt_1, 2, tmp))
            return -1;
        st.chip_cfg.lp_accel_mode = 0;
        return 0;
    }
    ...
    if (rate == 1)
        ...
        tmp[0] = INV_LPA_640HZ;
    if (MPU9250_Write_LEN( st.reg->lp_accel_odr, 1, tmp))
        return -1;
    tmp[0] = BIT_LPA_CYCLE;
    if (MPU9250_Write_LEN( st.reg->pwr_mgmt_1, 1, tmp))
        return -1;
#endif
...
int mpu_get_gyro_reg(short *data, unsigned long *timestamp)
{
    ...
    if (MPU_Read_LEN( st.reg->raw_gyro, 6, tmp))
        return -1;
...
int mpu_get_accel_reg(short *data, unsigned long *timestamp)
{
    ...
    if (MPU_Read_LEN( st.reg->raw_accel, 6, tmp))
        return -1;
    data[0] = (tmp[0] << 8) | tmp[1];
    data[1] = (tmp[2] << 8) | tmp[3];
    data[2] = (tmp[4] << 8) | tmp[5];
...
int mpu_get_temperature(long *data, unsigned long *timestamp)
{
    ...
    if (MPU_Read_LEN(st.reg->temp, 2, tmp))
        return -1;
    raw = (tmp[0] << 8) | tmp[1];
    if (timestamp)
        get_ms(timestamp);
...
int mpu_read_6500_accel_bias(long *accel_bias) {
	unsigned char data[6];
	if (MPU_Read_LEN( 0x77, 2, &data[0]))
		return -1;
	if (MPU_Read_LEN( 0x7A, 2, &data[2]))
		return -1;
	if (MPU_Read_LEN(0x7D, 2, &data[4]))
		return -1;
...
int mpu_read_6050_accel_bias(long *accel_bias) {
	unsigned char data[6];
	if (MPU_Read_LEN( 0x06, 2, &data[0]))
		return -1;
	if (MPU_Read_LEN( 0x08, 2, &data[2]))
		return -1;
	if (MPU_Read_LEN(0x0A, 2, &data[4]))
		return -1;
	accel_bias[0] = ((long)data[0]<<8) | data[1];
	accel_bias[1] = ((long)data[2]<<8) | data[3];
	accel_bias[2] = ((long)data[4]<<8) | data[5];
...
int mpu_read_6500_gyro_bias(long *gyro_bias) {
	unsigned char data[6];
	if (MPU_Read_LEN( 0x13, 2, &data[0]))
		return -1;
	if (MPU_Read_LEN(0x15, 2, &data[2]))
		return -1;
	if (MPU_Read_LEN( 0x17, 2, &data[4]))
		return -1;
...
int mpu_set_gyro_bias_reg(long *gyro_bias)
{
    ...
    if (MPU9250_Write_LEN( 0x13, 2, &data[0]))
        return -1;
    if (MPU9250_Write_LEN( 0x15, 2, &data[2]))
        return -1;
    if (MPU9250_Write_LEN( 0x17, 2, &data[4]))
        return -1;
    return 0;
}
...
int mpu_set_accel_bias_6050_reg(const long *accel_bias) {
    ...
    if (MPU9250_Write_LEN( 0x06, 2, &data[0]))
        return -1;
    if (MPU9250_Write_LEN(0x08, 2, &data[2]))
        return -1;
    if (MPU9250_Write_LEN(0x0A, 2, &data[4]))
        return -1;
...
int mpu_set_accel_bias_6500_reg(const long *accel_bias) {
    ...
    if (MPU9250_Write_LEN( 0x77, 2, &data[0]))
        return -1;
    if (MPU9250_Write_LEN(0x7A, 2, &data[2]))
        return -1;
    if (MPU9250_Write_LEN( 0x7D, 2, &data[4]))
        return -1;
...
int mpu_reset_fifo(void)
{
    ...
    data = 0;
    if (MPU9250_Write_Reg( st.reg->int_enable,data))
        return -1;
    if (MPU9250_Write_Reg( st.reg->fifo_en, data))
        return -1;
    if (MPU9250_Write_Reg(st.reg->user_ctrl, data))
        return -1;
    ...
    if (st.chip_cfg.dmp_on) {
        ...
        if (MPU9250_Write_Reg( st.reg->int_enable, data))
            return -1;
        if (MPU9250_Write_Reg(st.reg->fifo_en, st.chip_cfg.fifo_enable))
            return -1;
    }
    return 0;
}
...
int mpu_set_gyro_fsr(unsigned short fsr)
{
    ...
    if (st.chip_cfg.gyro_fsr == (data >> 3))
        return 0;
    if (MPU9250_Write_Reg( st.reg->gyro_cfg,  data))
        return -1;
...
int mpu_set_accel_fsr(unsigned char fsr)
{
    ...
    if (st.chip_cfg.accel_fsr == (data >> 3))
        return 0;
    if (MPU9250_Write_Reg( st.reg->accel_cfg, data))
        return -1;
...
int mpu_set_lpf(unsigned short lpf)
{
    ...
    if (st.chip_cfg.lpf == data)
        return 0;
    if (MPU9250_Write_Reg(st.reg->lpf, data))
        return -1;
...
int mpu_set_sample_rate(unsigned short rate)
{
    ...
    if (st.chip_cfg.dmp_on)
        ...
        data = 1000 / rate - 1;
        if (MPU9250_Write_Reg( st.reg->rate_div,  data))
            return -1;
...
int mpu_set_compass_sample_rate(unsigned short rate)
{
    ...
    if (MPU9250_Write_Reg( st.reg->s4_ctrl,div))
        return -1;
...
int mpu_set_sensors(unsigned char sensors)
{
    ...
    if (sensors & INV_XYZ_GYRO)
        ...
        data = BIT_SLEEP;
    if (MPU9250_Write_Reg( st.reg->pwr_mgmt_1,data)) {
    ...
    if (!(sensors & INV_XYZ_ACCEL))
        data |= BIT_STBY_XYZA;
    if (MPU9250_Write_Reg( st.reg->pwr_mgmt_2, data)) {
        st.chip_cfg.sensors = 0;
        return -1;
    }
    ...
    if (sensors & INV_XYZ_COMPASS)
        ...
        mpu_set_bypass(0);
#else
    if (MPU_Read_LEN( st.reg->user_ctrl, 1, &user_ctrl))
        return -1;
    ...
    if (st.chip_cfg.dmp_on)
        ...
        user_ctrl &= ~BIT_DMP_EN;
    if (MPU9250_Write_Reg( st.reg->s1_do,data))
        return -1;
    /* Enable/disable I2C master mode. */
    if (MPU9250_Write_Reg( st.reg->user_ctrl,user_ctrl))
        return -1;
#endif
#endif
...
int mpu_get_int_status(short *status)
{
    ...
    if (!st.chip_cfg.sensors)
        return -1;
    if (MPU_Read_LEN( st.reg->dmp_int_status, 2, tmp))
        return -1;
        ...
        unsigned char *sensors, unsigned char *more)
{
    ...
    if (MPU_Read_LEN( st.reg->fifo_count_h, 2, data))
        return -1;
...
//    log_i("FIFO count: %hd\n", fifo_count);
    if (fifo_count > (st.hw->max_fifo >> 1)) {
        ...
        if (MPU_Read_LEN( st.reg->int_status, 1, data))
            return -1;
    ...
    if (MPU_Read_LEN(st.reg->fifo_r_w, packet_size, data))
        return -1;
    ...
    unsigned char *more)
{
	...
	// printff(USART3, "33333\r\n", strlen("33333\r\n"));
    if (!st.chip_cfg.dmp_on)
        return -1;
    if (!st.chip_cfg.sensors)
        return -1;
// printff(USART3, "44444\r\n", strlen("44444\r\n"));
    if (MPU_Read_LEN( st.reg->fifo_count_h, 2, tmp))
        return -1;
    ...
    if (fifo_count > (st.hw->max_fifo >> 1)) {
        /* FIFO is 50% full, better check overflow bit. */
        if (MPU_Read_LEN( st.reg->int_status, 1, tmp))
            return -1;
    ...
    if (MPU_Read_LEN( st.reg->fifo_r_w, length, data))
        return -1;
...
int mpu_set_bypass(unsigned char bypass_on)
{
    ...
    if (bypass_on) {
        if (MPU_Read_LEN( st.reg->user_ctrl, 1, &tmp))
            return -1;
        tmp &= ~BIT_AUX_IF_EN;
        if (MPU9250_Write_Reg( st.reg->user_ctrl,  tmp))
            return -1;
        LL_mDelay(3);
        tmp = BIT_BYPASS_EN;
        ...
        if (MPU9250_Write_Reg(st.reg->int_pin_cfg, tmp))
            return -1;
    } else {
...
static int compass_self_test(void)
{
    ...
    if (MPU9250_Write_Reg( AKM_REG_CNTL, tmp[0]))
        return 0x07;
    tmp[0] = AKM_BIT_SELF_TEST;
    if (MPU9250_Write_Reg( AKM_REG_ASTC,  tmp[0]))
        goto AKM_restore;
    ...
    do {
        LL_mDelay(10);
        if (MPU_Read_LEN( AKM_REG_ST1, 1, tmp))
            goto AKM_restore;
    ...
    if (MPU_Read_LEN(AKM_REG_HXL, 6, tmp))
        goto AKM_restore;
...
AKM_restore:
    ...
    MPU9250_Write_Reg( AKM_REG_ASTC,  tmp[0]);
    tmp[0] = SUPPORTS_AK89xx_HIGH_SENS;
    MPU9250_Write_Reg( AKM_REG_CNTL, tmp[0]);
    mpu_set_bypass(0);
    return result;
}
#endif
...
static int get_st_biases(long *gyro, long *accel, unsigned char hw_test)
{
    ...
    if (MPU9250_Write_LEN(st.reg->pwr_mgmt_1, 2,data))
        return -1;
    LL_mDelay(200);
    data[0] = 0;
    if (MPU9250_Write_Reg( st.reg->int_enable,  data[0]))
        return -1;
    if (MPU9250_Write_Reg( st.reg->fifo_en, data[0]))
        return -1;
    if (MPU9250_Write_Reg( st.reg->pwr_mgmt_1, data[0]))
        return -1;
    if (MPU9250_Write_Reg( st.reg->i2c_mst,  data[0]))
        return -1;
    if (MPU9250_Write_Reg(st.reg->user_ctrl, data[0]))
        return -1;
    ...
    if (MPU9250_Write_Reg( st.reg->lpf,  data[0]))
        return -1;
    data[0] = st.test->reg_rate_div;
    if (MPU9250_Write_Reg(st.reg->rate_div, data[0]))
        return -1;
    if (hw_test)
        data[0] = st.test->reg_gyro_fsr | 0xE0;
    else
        data[0] = st.test->reg_gyro_fsr;
    if (MPU9250_Write_Reg(st.reg->gyro_cfg,  data[0]))
        return -1;
    ...
    if (MPU9250_Write_Reg(st.reg->accel_cfg,  data[0]))
        return -1;
    if (hw_test)
        LL_mDelay(200);
...
if (MPU_Read_LEN(st.reg->fifo_count_h, 2, data))
    ...
    for (ii = 0; ii < packet_count; ii++) {
        ...
        if (MPU_Read_LEN(st.reg->fifo_r_w, MAX_PACKET_LENGTH, data))
            return -1;
...
static int accel_6500_self_test(long *bias_regular, long *bias_st, int debug)
{
    ...
    if (MPU_Read_LEN( REG_6500_XA_ST_DATA, 3, regs)) {
    	if(debug)
...
static int gyro_6500_self_test(long *bias_regular, long *bias_st, int debug)
{
    ...
    if (MPU_Read_LEN( REG_6500_XG_ST_DATA, 3, regs)) {
    	if(debug)
    		log_i("Reading OTP Register Error.\n");
        return 0x07;
    }
...
static int get_st_6500_biases(long *gyro, long *accel, unsigned char hw_test, int debug)
{
    ...
    LL_mDelay(200);
    data[0] = 0;
    if (MPU9250_Write_LEN( st.reg->int_enable, 1, data))
        return -1;
    if (MPU9250_Write_LEN( st.reg->fifo_en, 1, data))
        return -1;
    if (MPU9250_Write_LEN( st.reg->pwr_mgmt_1,1,data))
        return -1;
    if (MPU9250_Write_LEN(st.reg->i2c_mst, 1, data))
        return -1;
    if (MPU9250_Write_Reg( st.reg->user_ctrl,  data[0]))
        return -1;
    data[0] = BIT_FIFO_RST | BIT_DMP_RST;
    ...
    LL_mDelay(test.wait_ms);  //wait 200ms for sensors to stabilize
    ...
    while (s < test.packet_thresh) {
    	LL_mDelay(test.sample_wait_ms); //wait 10ms to fill FIFO
		if (MPU_Read_LEN( st.reg->fifo_count_h, 2, data))
			return -1;
		fifo_count = (data[0] << 8) | data[1];
		packet_count = fifo_count / MAX_PACKET_LENGTH;
		...
		if (MPU_Read_LEN( st.reg->fifo_r_w, read_size, data))
						return -1;
		ind = 0;
		for (ii = 0; ii < packet_count; ii++) {
			short accel_cur[3], gyro_cur[3];
...
\
		  if (MPU9250_Write_LEN( st.reg->bank_sel,2,tmp))
        return -1;
		
    if (MPU9250_Write_LEN(st.reg->mem_r_w, length, data))
        return -1;
    return 0;
}
        ...
        unsigned char *data)
{
    ...
    if (MPU9250_Write_LEN( st.reg->bank_sel, 2, tmp))
        return -1;
    if (MPU_Read_LEN( st.reg->mem_r_w, length, data))
        return -1;
    return 0;
}
...
// printff(USART3, "111!!!\r\n",strlen("111!!!\r\n"));
    ...
    for (ii = 0; ii < length; ii += this_write) 
        ...
        if (mpu_write_mem(ii, this_write, (unsigned char*)&firmware[ii]))
            return -1;
		//		 printff(USART3, "333aa!!!\r\n",strlen("333aa!!!\r\n"));
        if (mpu_read_mem(ii, this_write, cur))
            return -1;
		//		 printff(USART3, "444aa!!!\r\n",strlen("444aa!!!\r\n"));
    ...
    if (MPU9250_Write_LEN( st.reg->prgm_start_h, 2, tmp))
        return -1;
	//	 printff(USART3, "666!!!\r\n",strlen("666!!!\r\n"));
...
static int setup_compass(void)
{
    ...
    for (akm_addr = 0x0C; akm_addr <= 0x0F; akm_addr++) {
        int result;
        result = MPU_Read_LEN( AKM_REG_WHOAMI, 1, data);
    ...
    if (MPU9250_Write_LEN( AKM_REG_CNTL, 1, data))
        return -1;
    LL_mDelay(1);
    ...
    if (MPU_Read_LEN( AKM_REG_ASAX, 3, data))
        return -1;
    ...
    if (MPU9250_Write_LEN( st.reg->s0_addr, 1, data))
        return -1;

    /* Compass reads start at this register. */
    data[0] = AKM_REG_ST1;
    if (MPU9250_Write_LEN( st.reg->s0_reg, 1, data))
        return -1;

    /* Enable slave 0, 8-byte reads. */
    data[0] = BIT_SLAVE_EN | 8;
    if (MPU9250_Write_LEN( st.reg->s0_ctrl, 1, data))
        return -1;

    /* Slave 1 changes AKM measurement mode. */
    data[0] = st.chip_cfg.compass_addr;
    if (MPU9250_Write_LEN( st.reg->s1_addr, 1, data))
        return -1;

    /* AKM measurement mode register. */
    data[0] = AKM_REG_CNTL;
    if (MPU9250_Write_LEN( st.reg->s1_reg, 1, data))
        return -1;

    /* Enable slave 1, 1-byte writes. */
    data[0] = BIT_SLAVE_EN | 1;
    if (MPU9250_Write_LEN( st.reg->s1_ctrl, 1, data))
        return -1;

    /* Set slave 1 data. */
    data[0] = AKM_SINGLE_MEASUREMENT;
    if (MPU9250_Write_LEN( st.reg->s1_do, 1, data))
        return -1;

    /* Trigger slave 0 and slave 1 actions at each sample. */
    data[0] = 0x03;
    if (MPU9250_Write_LEN(st.reg->i2c_delay_ctrl, 1, data))
        return -1;
...
//    if (!(st.chip_cfg.sensors & INV_XYZ_COMPASS))
//        return -1;
//printff(USART3, "1111111111111111111111111!!\r\n",strlen("1111111111111111111111111!!\r\n"));
//#ifdef AK89xx_BYPASS
...
//        return -1;
//	printff(USART3, "22222222222222222!!\r\n",strlen("22222222222222222!!\r\n"));	
//#endif
...
//        return -2;
//			printff(USART3, "333333333333333!!\r\n",strlen("333333333333333!!\r\n"));	
//    if (tmp[7] & AKM_OVERFLOW)
//        return -3;
//			printff(USART3, "4444444444444444!!\r\n",strlen("4444444444444444!!\r\n"));	
//#endif
...
//#endif
		...
		READ_MPU9250_MAG();
		data[0] = mpu_value.Mag[0];
    data[1] = mpu_value.Mag[1];
		data[2] = mpu_value.Mag[2];
	  if (timestamp)
       get_ms(timestamp);
    return 0;
}
    ...
    unsigned short lpa_freq)
{
    ...
    if (lpa_freq) {
        ...
        if (MPU9250_Write_LEN(st.reg->user_ctrl, 3, data))
            goto lp_int_restore;

        /* Set motion threshold. */
        data[0] = thresh_hw;
        if (MPU9250_Write_LEN( st.reg->motion_thr, 1, data))
            goto lp_int_restore;
        ...
        if (MPU9250_Write_LEN(st.reg->lp_accel_odr, 1, data))
            goto lp_int_restore;

        /* Enable motion interrupt (MPU6500 version). */
        data[0] = BITS_WOM_EN;
        if (MPU9250_Write_LEN( st.reg->accel_intel, 1, data))
            goto lp_int_restore;
...
uint8_t mpu_dmp_init(void)
{
	...
	{	 
		//    printff(USART3, "5555555555555!!!\r\n",strlen("5555555555555!!!\r\n"));	   
        res=inv_init_mpl();     //初始化MPL
    	//	printff(USART3, "77777777777777!!!\r\n",strlen("77777777777777!!!\r\n"));	   
        if(res)return 1;
	    //	printff(USART3, "66666666666666!!!\r\n",strlen("66666666666666!!!\r\n"));	   
        ...
        inv_enable_eMPL_outputs();
			...
			//	printff(USART3, "8888888888888888!!!\r\n",strlen("8888888888888888!!!\r\n"));	   
				if(res)return 1;
			//	printff(USART3, "99999999999!!!\r\n",strlen("99999999999!!!\r\n"));	 
				res=mpu_set_sensors(INV_XYZ_GYRO|INV_XYZ_ACCEL|INV_XYZ_COMPASS);//设置所需要的传感器
				if(res)return 2; 
			//	printff(USART3, "aaaaaaaaaaaa!!!\r\n",strlen("aaaaaaaaaaaa!!!\r\n"));	 
				res=mpu_configure_fifo(INV_XYZ_GYRO | INV_XYZ_ACCEL);   //设置FIFO
				if(res)return 3; 
			//	printff(USART3, "bbbbbbbbbbbbbbb!!!\r\n",strlen("bbbbbbbbbbbbbbb!!!\r\n"));	 
				res=mpu_set_sample_rate(DEFAULT_MPU_HZ);	            //设置采样率
				if(res)return 4; 
		  //	printff(USART3, "cccccccccccccc!!!\r\n",strlen("cccccccccccccc!!!\r\n"));	 
        res=mpu_set_compass_sample_rate(1000/COMPASS_READ_MS);  //设置磁力计采样率
        if(res)return 5;
			//  printff(USART3, "dddddddddddddddd!!!\r\n",strlen("dddddddddddddddd!!!\r\n"));	 	
   ...
   // printff(USART3, "eeeeeeeeeeee!!!\r\n",strlen("eeeeeeeeeeee!!!\r\n"));	 
		res=dmp_load_motion_driver_firmware();		             //加载dmp固件
		//printff(USART3, "fffffffffffffffffffffff!!!\r\n",strlen("fffffffffffffffffffffff!!!\r\n"));	 	
		if(res)return 6; 
	 // printff(USART3, "gggggggggggggggggggg!!!\r\n",strlen("gggggggggggggggggggg!!!\r\n"));	
		...
		if(res)return 9;   
	//	printff(USART3, "hhhhhhhhhhhhhh!!!\r\n",strlen("hhhhhhhhhhhhhh!!!\r\n"));	 	
		res=run_self_test();		//自检
		if(res)return 10;    
	//printff(USART3, "iiiiiiiiiiiiii!!!\r\n",strlen("iiiiiiiiiiiiii!!!\r\n"));	 
		res=mpu_set_dmp_state(1);	//使能DMP
		if(res)return 11;     
	//	printff(USART3, "jjjjjjjjjjjjjjj!!!\r\n",strlen("jjjjjjjjjjjjjjj!!!\r\n"));	 
...
uint8_t mpu_mpl_get_data(float *pitch,float *roll,float *yaw)
{
	...
	char compass_buf[30]={0};
		char compass_buf1[30]={0};
			char compass_buf2[30]={0};
    ...
    {
			printff(USART3, "\r\nget compass ok\r\n", strlen("\r\nget compass ok\r\n"));
        compass[0]=(long)accel[0];
        compass[1]=(long)accel[1];
        compass[2]=(long)accel[2];
        inv_build_compass(compass,0,sensor_timestamp); //把磁力计值发给MPL
    }
		
		sprintf(compass_buf,"%ld",compass[0]);
		sprintf(compass_buf1,"%ld",compass[1]);
		sprintf(compass_buf2,"%ld",compass[2]);
	  printff(USART3, "--------------------------------\r\n", strlen("--------------------------------\r\n"));
		printff(USART3, compass_buf,strlen(compass_buf));	 
    printff(USART3, "\r\n", strlen("\r\n"));		
		printff(USART3, compass_buf1,strlen(compass_buf1));
		printff(USART3, "\r\n", strlen("\r\n"));
		printff(USART3, compass_buf2,strlen(compass_buf2));
		printff(USART3, "\r\n--------------------------------\r\n", strlen("\r\n--------------------------------\r\n"));
...
//				for(i=0;i<4;i++)
//				{
//				   TEST_BUF[i]=(char)*temp; 
//				   temp++;
//				}
//		memcpy(TEST_BUF,&accel,20);
